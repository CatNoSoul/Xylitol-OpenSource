        ...
        if (currentModule != null) {
            ...
            for (i = 0; i < currentModule.getValues().size() && CStartY <= this.startY + 300.0f; ++i) {
                ...
                if (cValue instanceof ModeValue) {
                    ...
                    int next_str = mode.ordinal() - 1 < 0 ? ((ModeValue)cValue).getModes().length - 1 : mode.ordinal() - 1;
                    int next_str_2 = mode.ordinal() + 1 >= ((ModeValue)cValue).getModes().length ? 0 : mode.ordinal() + 1;
                    ...
                    for (int i1 = 0; i1 < 2; ++i1) {
                        FontManager.Tahoma18.drawStringWithShadow(((ModeValue)cValue).getModes()[next_str].name().substring(((ModeValue)cValue).getModes()[next_str].name().length() - 2 + i1, ((ModeValue)cValue).getModes()[next_str].name().length() + i1 - 1), x - 3.0f + (float)(i1 * FontManager.Tahoma18.getStringWidth(((ModeValue)cValue).getModes()[next_str].name().substring(((ModeValue)cValue).getModes()[next_str].name().length() - 3 + i1, ((ModeValue)cValue).getModes()[next_str].name().length() + i1 - 2))), CStartY + 2.0f, this.getColor(255, 255, 255, 80 / (3 - (i1 + 1))));
                        FontManager.Tahoma18.drawStringWithShadow(((ModeValue)cValue).getModes()[next_str_2].name().substring(i1, i1 + 1), x + 79.0f + (float)(i1 > 0 ? i1 * FontManager.Tahoma18.getStringWidth(((ModeValue)cValue).getModes()[next_str_2].name().substring(i1 - 1, i1)) : 0), CStartY + 2.0f, this.getColor(255, 255, 255, 80 / (i1 + 1)));
                    }
                    FontManager.Tahoma18.drawStringWithShadow(((Enum)cValue.getValue()).name(), x + 42.5f - (float)(FontManager.Tahoma18.getStringWidth(((Enum)((ModeValue)cValue).getValue()).name()) / 2), CStartY + 2.0f, this.getColor(255, 255, 255, 255));
                    if (this.isStringHovered(x, CStartY - 5.0f, x + 100.0f, CStartY + 15.0f, mouseX, mouseY)) {
                        int next;
                        if (!this.previousmouse && Mouse.isButtonDown(0)) {
                            next = mode.ordinal() + 1 >= ((ModeValue)cValue).getModes().length ? 0 : mode.ordinal() + 1;
                           // cValue.setValue(((ModeValue)cValue).getModes()[next]);
                            this.previousmouse = true;
                        }
                        if (Mouse.isButtonDown(1) && !this.Rpreviousmouse) {
                            next = mode.ordinal() - 1 < 0 ? ((ModeValue)cValue).getModes().length - 1 : mode.ordinal() - 1;
                           // cValue.setValue(((ModeValue)cValue).getModes()[next]);
